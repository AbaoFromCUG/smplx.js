...
public:
    ...
    DenseMatrix() { data = Mat(); }

    DenseMatrix(int m, int n) { data = Mat::Zero(m, n); }
    ...
    void set(int i, int j, const T &s) { data(i, j) = s; }
    ...
    double normSqr() const { return data.squaredNorm(); }

    double l1Norm() const { return data.template lpNorm<1>(); }

    double lInfNorm() const { return data.template lpNorm<Eigen::Infinity>(); }
    ...
    double det() const { return data.determinant(); }
    ...
    {
        assert(rows() >= i + block.rows() && cols() >= j + block.cols()
               && "The matrix doens't fit");
    ...
    Mat copy() const { return data; }

    Mat &toEigen() { return data; }

    auto mul(const T &s) { return DenseMatrix<T>(data * s); }

    void mulSelf(const T &s) { data *= s; }

    auto div(const T &s) { return DenseMatrix<T>(data / s); }

    void divSelf(const T &s) { data /= s; }

    DenseMatrix<T> matAdd(const DenseMatrix<T> *B) { return DenseMatrix<T>(data + B->data); }

    void matAddSelf(const DenseMatrix<T> *B) { data += B->data; }

    DenseMatrix<T> matSub(const DenseMatrix<T> *B) { return DenseMatrix<T>(data - B->data); }

    void matSubSelf(const DenseMatrix<T> *B) { data -= B->data; }

    DenseMatrix<T> matMul(const DenseMatrix<T> *B) { return DenseMatrix<T>(data * B->data); }

    void matMulSelf(const DenseMatrix<T> *B) { data *= B->data; }
    ...
    DenseMatrix<T> transpose() const { return DenseMatrix<T>(data.transpose()); }
    ...
    DenseMatrix<T> inverse() const { return DenseMatrix<T>(data.inverse()); }

    DenseMatrix<T> conjugate() const { return DenseMatrix<T>(data.conjugate()); }
        ...
        for (int i = 0; i < rows; i++) {
            ...
            for (int j = 0; j < cols; j++) {
                if (std::is_same<T, std::complex<double>>::value) {
                    result += string_format("%.2f + %.2fi", std::real(get(i, j)),
                                            std::imag(get(i, j)));
                } else {
                    result += string_format("%.2f", std::real(get(i, j)));
                }
                result += (j < cols - 1 ? ", " : "");
            }
            result += ((i < rows - 1 ? "]" : "]]"));
    ...
    static DenseMatrix<T> identity(int m, int n) { return DenseMatrix<T>(Mat::Identity(m, n)); }

    static DenseMatrix<T> ones(int m, int n) { return DenseMatrix<T>(Mat::Ones(m, n)); }
    ...
    static DenseMatrix<T> random(int m, int n) { return DenseMatrix<T>(Mat::Random(m, n)); }
