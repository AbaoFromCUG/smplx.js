...
public:
  ...
  T dot(const DenseMatrix<T> &B) const {
    assert(length() == B.length() &&
           "The two matrices must be same length vectors");
    double dot = 0;
  ...
  DenseMatrix<T> mul(const T &s) { return DenseMatrix<T>(data * s); }

  void mulSelf(const T &s) { data *= s; }

  DenseMatrix<T> div(const T &s) { return DenseMatrix<T>(data / s); }
